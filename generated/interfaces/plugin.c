    ...
    *private_data = ctx;
    ...
    srpc_operational_t oper[] = {
        ...
        },
        {
            ...
            interfaces_subscription_operational_interfaces_state_interface,
        },
    };
    ...
    for (size_t i = 0; i < ARRAY_SIZE(module_changes); i++)
    {
        ...
        if (change->cb)
        {
            error = sr_module_change_subscribe(running_session, BASE_YANG_MODEL, change->path, change->cb,
                                               *private_data, 0, SR_SUBSCR_DEFAULT, &subscription);
            if (error)
            {
                SRPLG_LOG_ERR(PLUGIN_NAME, "sr_module_change_subscribe() error for \"%s\" (%d): %s", change->path,
    ...
    for (size_t i = 0; i < ARRAY_SIZE(rpcs); i++)
    {
        ...
        if (rpc->cb)
        {
            error = sr_rpc_subscribe(running_session, rpc->path, rpc->cb, *private_data, 0, SR_SUBSCR_DEFAULT,
                                     &subscription);
            if (error)
            {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(oper); i++)
    {
        ...
        if (op->cb)
        {
            error = sr_oper_get_subscribe(running_session, BASE_YANG_MODEL, op->path, op->cb, NULL, SR_SUBSCR_DEFAULT,
                                          &subscription);
            if (error)
            {
