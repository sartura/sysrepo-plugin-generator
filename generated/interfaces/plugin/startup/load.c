...
static int interfaces_startup_load_interface(void *priv, sr_session_ctx_t *session, const struct ly_ctx *ly_ctx,
...
int interfaces_startup_load(interfaces_ctx_t *ctx, sr_session_ctx_t *session)
{
    ...
    error = srpc_ly_tree_create_container(ly_ctx, NULL, &root_node, INTERFACES_INTERFACES_YANG_PATH);
    if (error)
    {
